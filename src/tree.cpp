...
void Tree::Render(int i)
{
    ...
    bool node_open = ImGui::TreeNode("Entity", "%s_%i", "n", i);
...
void Tree::Render(flecs::entity e)
{
    int id = e.id();
    ImGui::PushID(id);
    ...
    ImGui::Selectable
    bool node_open = ImGui::TreeNode("_Entity", "%i %s", id, e.name());
...
void Tree::Render()
{
    if (ImGui::Begin(m_window_name, &m_is_open))
    {
        ...
        if (ImGui::BeginTable("_TreeTable", 1, ImGuiTableFlags_BordersOuter | ImGuiTableFlags_Resizable | ImGuiTableFlags_ScrollY))
        {
            ...
            flecs::filter<Transform> f = m_engine->m_world.filter<Transform>();
            
            f.each([this](flecs::entity entity, Transform& t)
            {
                Render(entity);
            });
            ...
            //     // ImGui::TableSetColumnIndex(0);
            //     // ImGui::AlignTextToFramePadding();
            //     // bool node_open = ImGui::TreeNode(std::to_string(i).c_str());
